...
def resHymns():
    ...
    cHeading = Font(family="CS Avva Shenouda", size=40)
    englishFont = Font(family="Helvetica", size=20)
    big = Font(size=20)
    ...
    english_KA = Label(res,
                       ...
                       font=englishFont)
    arabic_KA = Label(res,
                      text="المسيح قام من بين الأموات، بالموت داس الموت، والذين في القبور، أنعم لهم بالحياة الأبدية." + "\n" + "المجد للآب والإبن والروح القدس، الآن وكل أوان وإلى دهر الدهور. آمين.",
                      font=big)
    ...
    arabic_KA.pack()
    audio()
    btmImg = ImageTk.PhotoImage(Image.open("pic.png"))
    btmLab = Label(res, image=topImg)
    btmLab.pack()
    vs = Scrollbar(res)
    vs.pack(side=RIGHT, fill=Y)
    hs = Scrollbar(res, orient="horizontal")
